...
public class ShipController : MonoBehaviour {
	...
	void InputFixedUpdate()
	{
		...
		if(adjustThrustZ)
		{
			if(!impulseMode)
			{
				ship.AddForce(transform.forward * (thrust.z * throttle) * 10000, ForceMode.Force);
			}
			else if(impulseTimer >= impulseCoolDown)
			{
				ship.AddForce(transform.forward * (thrust.z * throttle)* 10000, ForceMode.Impulse);
				impulseTimer = 0.0f;
			}
		}
		...
		if (adjustThrustX)
		{
			if(!impulseMode)
			{
				ship.AddForce(transform.right * (thrust.x * throttle)* 10000, ForceMode.Force);
			}
			else if (impulseTimer >= impulseCoolDown)
			{
				ship.AddForce(transform.right * (thrust.x * throttle)* 10000, ForceMode.Impulse);
				impulseTimer = 0.0f;
			}
		}
		...
		if(adjustThrustY)
		{
			if(!impulseMode)
			{
				ship.AddForce(transform.up * (throttle * thrust.y)* 10000, ForceMode.Force);
			}
			if (impulseMode && impulseTimer >= impulseCoolDown)
			{
				ship.AddForce(transform.up * (throttle * thrust.y)* 10000, ForceMode.Impulse);
				impulseTimer = 0.0f;
			}
		}
	}
